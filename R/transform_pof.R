...
#' @param pof_tables_names A character vector containing name of each table of
#' POF.
...
#' }
#' @export
transform_pof <- function(pof_data, pof_tables_names, pof_year){
  ...
  data <- purrr::map2(
    data_base,
    names(data_base),
    ~transform_pof(.x, .y, data_base_year)
  )

  # Creating DataFrame POF by UC ------------------------------------------------
  ## For POF 2018
  if(data_base_year == 2018){
