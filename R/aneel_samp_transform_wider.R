...
aneel_samp_transform_wider <- function(dir = "data_raw/aneel_samp",
  ...
  aneel_samp <- readr::read_csv(file_name) |>
    ...
    dplyr::mutate(
      .by = c(dat_competencia, sig_agente_distribuidora),
      energia_te_dist = sum(energia_te_k_wh, na.rm = T),
      tarifa_sum_dist = TE_com_imposto * energia_te_dist
    ) |>
    dplyr::mutate(
      .by = c(dat_competencia, uf, dsc_sub_grupo_tarifario, dsc_classe_consumo_mercado),
      TE_sem_imposto_media_uf = mean(TE_sem_imposto, na.rm = T),
      TE_com_imposto_media_uf = mean(TE_com_imposto, na.rm = T),
      TE_social_sem_imposto_media_uf = mean(TE_social_sem_imposto, na.rm = T),
